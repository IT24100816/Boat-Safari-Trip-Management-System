...
<head>
  ...
  <title>Booking Manager Profile - BlueWave Safaris</title>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
      ...
      <div class="relative bg-gradient-to-b from-blue-50 to-white rounded-2xl shadow-2xl overflow-hidden transform hover:scale-[1.02] transition-all duration-300 border border-gray-200">
        ...
        <div class="relative flex flex-col items-center text-center pt-20 pb-8 px-6">
          ...
          <h3 class="text-2xl font-bold text-gray-800 mt-4">${bookingManager.firstName} ${bookingManager.lastName}</h3>
          <p class="text-gray-500 text-sm mt-1">${bookingManager.email}</p>
          <span class="mt-2 px-3 py-1 bg-blue-100 text-blue-600 rounded-full text-xs font-medium shadow-sm uppercase tracking-wide">
            ${bookingManager.role != null ? bookingManager.role : "Booking Manager"}
          </span>
          ...
          <div class="space-y-3 text-sm text-left w-full px-3">
            <p class="flex items-center">
              ...
              <span><strong>NIC:</strong> ${bookingManager.nic}</span>
            </p>
            <p class="flex items-center">
              <i class="fas fa-birthday-cake text-blue-500 mr-3"></i>
              <span><strong>Age:</strong> ${bookingManager.age}</span>
            </p>
            <p class="flex items-center">
              <i class="fas fa-phone text-blue-500 mr-3"></i>
              <span><strong>Phone:</strong> ${bookingManager.phone}</span>
            </p>
          </div>
      ...
      <div class="lg:col-span-2 space-y-8">
        ...
        <div id="edit" class="bg-white rounded-2xl shadow-xl p-8 card">
          ...
          <form action="/booking-manager/profile/edit" method="post" onsubmit="return validateForm()" class="space-y-6">
            <input type="hidden" name="id" value="${bookingManager.id}" />

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label for="firstName" class="block text-sm font-medium text-gray-600 mb-1">First Name</label>
                <input type="text" id="firstName" name="firstName" value="${bookingManager.firstName}"
              ...
              <div>
                ...
                <input type="text" id="lastName" name="lastName" value="${bookingManager.lastName}"
                       class="w-full p-3 border border-gray-200 rounded focus-ring" required />
              </div>
              <div>
                <label for="email" class="block text-sm font-medium text-gray-600 mb-1">Email</label>
                <input type="email" id="email" name="email" value="${bookingManager.email}"
              ...
              <div>
                ...
                <input type="text" id="nic" name="nic" value="${bookingManager.nic}"
                       class="w-full p-3 border border-gray-200 rounded focus-ring" required />
              </div>
              <div>
                <label for="age" class="block text-sm font-medium text-gray-600 mb-1">Age</label>
                <input type="number" id="age" name="age" value="${bookingManager.age}"
              ...
              <div>
                ...
                <input type="text" id="phone" name="phone" value="${bookingManager.phone}"
        ...
        <div id="passwordForm" class="bg-white rounded-2xl shadow-xl p-8 card hidden">
          ...
          <form action="/booking-manager/profile/change-password" method="post" onsubmit="return validatePasswordChange()" class="space-y-6">
            <input type="hidden" name="id" value="${bookingManager.id}" />
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 card border border-red-200">
          ...
          <form id="deleteForm" action="/booking-manager/profile/delete" method="post" onsubmit="return validateDelete()" class="mt-6 space-y-4 hidden">
            <input type="hidden" name="id" value="${bookingManager.id}" />
            <div>
